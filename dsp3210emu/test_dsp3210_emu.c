/*
 * test_dsp3210_emu.c — unit tests for the DSP3210 reference emulator.
 *
...
{
    if (r == 0) return 0;
    if (r <= 14) return (unsigned)r;
    if (r <= 19) return (unsigned)r + 2;
    return (unsigned)r + 4;
}
#define RC_PC   15u
#define RC_MIN  22u   /* -n */
...
{ return ((lng ? 0x25u : 0x05u) << 26) | (rd << 21) | (rs3 << 16)
       | ((uint32_t)n & 0xFFFFu); }

static uint32_t e_alur(int lng, unsigned f, unsigned rd, unsigned rs1,
                       unsigned c, unsigned rs2)                 /* 6a/6b */
{ return ((uint32_t)lng << 31) | (0x0Cu << 25) | (f << 21) | (rd << 16)
       | (rs1 << 11) | (c << 5) | rs2; }

static uint32_t e_alui(int lng, unsigned f, unsigned rd, int n)  /* 6c/6d */
{ return ((uint32_t)lng << 31) | (0x0Du << 25) | (f << 21) | (rd << 16)
       | ((uint32_t)n & 0xFFFFu); }

static uint32_t e_mvdir(unsigned t, unsigned w, unsigned rh, unsigned L)
{ return (0x07u << 26) | (t << 24) | (w << 21) | (rh << 16) | L; } /* 7a */

static uint32_t e_mvind(unsigned t, unsigned w, unsigned rh, unsigned rp,
                        unsigned ri)                             /* 7c */
{ return (0x27u << 26) | (t << 24) | (w << 21) | (rh << 16) | (rp << 11)
       | ri; }
...
{ return (5u << 29) | (((m >> 16) & 0xFFu) << 21) | (rb << 16)
       | (m & 0xFFFFu); }

static uint32_t e_set24(unsigned rd, uint32_t m)                 /* 8b */
{ return (6u << 29) | (((m >> 16) & 0xFFu) << 21) | (rd << 16)
       | (m & 0xFFFFu); }

static uint32_t e_call24(unsigned rm, uint32_t m)                /* 8c */
{ return (7u << 29) | (((m >> 16) & 0xFFu) << 21) | (rm << 16)
       | (m & 0xFFFFu); }

/* DA fields: X/Y/Z 7-bit p:i */
static unsigned DF(unsigned p, unsigned i) { return (p << 3) | i; }
#define DF_ACC(n)  DF(0, n)
#define DF_NOWR    0x07u

static uint32_t e_damac(unsigned fmt, unsigned m, unsigned fs, unsigned ss,
...
/* ALU F codes */
enum { F_ADD = 0, F_SHL = 1, F_RSUB = 2, F_CRADD = 3, F_SUB = 4,
       F_ANDC = 6, F_CMP = 7, F_XOR = 8, F_ROR = 9, F_OR = 10,
       F_ROL = 11, F_SHR = 12, F_ASR = 13, F_AND = 14, F_BTST = 15 };
/* condition codes */
enum { C_FALSE = 0, C_TRUE = 1, C_PL = 2, C_MI = 3, C_NE = 4, C_EQ = 5,
       C_CS = 9 };
/* W field */
enum { W_BYTE = 0, W_CHAR = 1, W_USHORT = 2, W_SHORT = 3, W_HBYTE = 4,
       W_LONG = 7 };
...
static void prog(const uint32_t *words, int n) { prog_at(0, words, n); }
...
static void steps(int n) { while (n--) dsp3210_step(&S); }
...
static void test_add_flags(void)
{
    ...
    const uint32_t q[] = {
        e_set24(RC(1), 0),
        e_alui(1, F_SUB, RC(1), 1),             /* r1 = r1 - 1 = -1 */
...
{
    const uint32_t p[] = {
        e_add3(0, RC(1), RC(0), 5),
        e_alui(1, F_CMP, RC(1), 7),             /* r1 - 7: borrow */
    };
    prog(p, 2);
    steps(2);
...
static void test_do_loops(void)
{
    const uint32_t p[] = {
        ...
        e_add3(1, RC(3), RC(3), 2),
        e_add3(0, RC(4), RC(0), 7),             /* after loop */
    };
    prog(p, 4);
    steps(1 + 8 + 1);
    CHECK(S.r[2] == 4);
...
static void test_direct_moves_window(void)
{
    ...
    const uint32_t p[] = {
        e_add3(0, RC(1), RC(0), 0x1234),
        e_mvdir(1, W_LONG, RC(1), 0xE000),      /* *0xe000 = r1 */
...
static void test_shifts(void)
{
    const uint32_t p[] = {
        e_set24(RC(1), 0x8001),
        e_alui(1, F_SHL, RC(1), 16),            /* 0x80010000 */
...
static void test_da_int_float(void)
{
    ...
    const uint32_t p[] = {
        e_set24(RC(1), 0x100),
        e_set24(RC(2), 0x104),
        e_daspec(2, 0, DF(1, 7), DF_NOWR),      /* float16 */
...
static void test_da_ieee_dsp(void)
{
    ...
    const uint32_t p[] = {
        e_set24(RC(1), 0x100),
        e_set24(RC(2), 0x104),
        e_daspec(13, 0, DF(1, 0), DF_NOWR),     /* dsp */
...
static void test_da_ifalt(void)
{
    const uint32_t p[] = {
        /* a0 = a1 - a2 → sets N */
        e_damac(1, 5, 0, 1, 0, DF_ACC(2), DF_ACC(1), DF_NOWR),
        e_daspec(5, 3, DF_ACC(1), DF_NOWR),     /* a3 = ifalt(a1) */
    };
    prog(p, 2);
    dsp3210_acc_set(&S, 1, 1.0); dsp3210_acc_set(&S, 2, 5.0); dsp3210_acc_set(&S, 3, 99.0);
    steps(2);
    CHECK(dsp3210_acc_get(&S, 0) == -4.0);
    CHECK(dsp3210_acc_get(&S, 3) == 1.0);                       /* N was set → loaded */

    prog(p, 2);
    dsp3210_acc_set(&S, 1, 9.0); dsp3210_acc_set(&S, 2, 5.0); dsp3210_acc_set(&S, 3, 99.0);
    steps(2);
