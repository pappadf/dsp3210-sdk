...
static int da_operand(sb *s, unsigned f, int is_z)
{
    ...
    default: sb_fmt(s, "++r%u", 14 + i); break;   /* 1-5 -> r15-r19 */
    }
    return 1;
}
...
static const char *const da_gfunc[16] = {
...
enum {
    F_ADD = 0, F_SHL_N = 1, F_RSUB = 2, F_CRADD = 3, F_SUB = 4,
    F_RES5 = 5, F_ANDC = 6, F_CMP = 7, F_XOR = 8, F_ROR = 9,
    F_OR = 10, F_ROL = 11, F_SHR_N = 12, F_ASR_N = 13, F_AND = 14,
    F_BTST = 15
};
...
static void dis_ca_alu_reg(uint32_t w, dsp3210_insn *ins, sb *s)
{
    ...
    /* no-store forms */
    if (f == F_CMP || f == F_BTST) {
        sb_fmt(s, "%s%s %s %s", size, ca_reg_name[rs1],
               (f == F_CMP) ? "-" : "&", ca_reg_name[rs2]);
        return;
    }

    /* stack-pointer bump: sp = sp{++,--} moves by 4, not 1 (IM INCR/DECR) */
    if (f == F_ADD && rd == RC_SP && rs1 == RC_SP
        && (rs2 == RC_PLUS || rs2 == RC_MINUS)) {
    ...
    case F_ADD:
        if (rs2 == RC_PLUS || rs2 == RC_MINUS) {
            sb_fmt(s, "%s %c 1", ca_reg_name[rs1],
                   rs2 == RC_PLUS ? '+' : '-');
    ...
    case F_ASR_N:
        if (rs2 == RC_PLUS)
            ...
            sb_fmt(s, "%s %s %s", ca_reg_name[rs1], f_op_sym[f],
                   ca_reg_name[rs2]);
        break;
    default:   /* #, &~, ^, |, & */
        sb_fmt(s, "%s %s %s", ca_reg_name[rs1], f_op_sym[f],
...
static void dis_ca_alu_imm(uint32_t w, dsp3210_insn *ins, sb *s)
{
    ...
    /* no-store forms */
    if (f == F_CMP || f == F_BTST) {
        sb_fmt(s, "%s%s %s ", size, ca_reg_name[rd],
               (f == F_CMP) ? "-" : "&");
        sb_imm(s, n);
        return;
    }
    ...
    case F_ASR_N:                                   /* 5 LSBs of N used */
        sb_fmt(s, "%s %s %u", ca_reg_name[rd], f_op_sym[f],
               (unsigned)(n & 31));
        break;
    case F_ROR:
...
static void dis_ca_move(uint32_t w, dsp3210_insn *ins, sb *s)
{
    ...
    if (!io && bits(w, 10, 10)) {
        ...
        if (t == 1 && rh == RC_R0 && ior == 10) {
            if (wsz == 7) { sb_puts(s, "waiti");  return; }
            if (wsz == 3) { sb_puts(s, "bkpt");   return; }
            if (wsz == 0) { sb_puts(s, "sftrst"); return; }
        }
        if (t == 0)
    ...
    /* 7c: rH <-> MEM   |   7d: iorH <-> MEM */
    {
        ...
        const char *regn = io ? ior_name(rh, tmp, sizeof tmp)
                              : ca_reg_name[rh];
...
int dsp3210_disassemble(uint32_t w, uint32_t addr, dsp3210_insn *ins)
{
    ...
    case 0x24: {                       /* 4b: rD = rS <<| N */
        ...
        sb_fmt(&s, "%s = %s <<| 0x%x", ca_reg_name[rd], ca_reg_name[rs],
               (unsigned)bits(w, 15, 0));
        break;
    }
...
{
    switch (status) {
    case DSP3210_OK:       return "ok";
    case DSP3210_ILLEGAL:  return "illegal";
    case DSP3210_RESERVED: return "reserved";
    default:               return "?";
    }
}
