 ...
 *      corroborate.)
 *   2. Words verified against real DSP3210 systems (recovered from the
 *      Macintosh Quadra 840AV ROM), e.g. 9CFA2817 = "r22 = *r5++".
...
static uint32_t da(unsigned fmt, unsigned m, unsigned f, unsigned s,
                   unsigned n, unsigned x, unsigned y, unsigned z)
{
    return (fmt << 29) | (m << 26) | (f << 24) | (s << 23) | (n << 21)
         | (x << 14) | (y << 7) | z;
}
...
static void expect_at(uint32_t word, uint32_t addr, const char *want,
                      int want_status)
{
    dsp3210_insn ins;
...
static void expect_target(uint32_t word, uint32_t addr, const char *want,
                          uint32_t target)
{
    dsp3210_insn ins;
...
int main(void)
{
    ...
    /* ---- moves (7a-7d) — IM LOAD/STORE examples + hardware-verified words ---- */
    expect(0x1C4A00AA, "r10 = (ushort) *0xaa");        /* IM LOAD (2) */
    expect(0x1CE100AA, "r1 = *0xaa");
    expect(0x1D651234, "*0x1234 = (short) r5");
    expect(0x9CE41017, "r4 = *r2++");                  /* hw-verified */
    expect(0x9CE41012, "r4 = *r2++r16");
    expect(0x9D041817, "*r3++ = (byte) r4");           /* hw-verified */
    expect(0x9CFA2817, "r22 = *r5++");                 /* hw-verified */
    expect(0x9DE00000, "*r0 = r0");                    /* hw-verified */
    ...
    expect(0x9D61040C, "pcw = (short) r1");            /* hw-verified */
    expect(0x9CE7040E, "r7 = dauc");
    expect(0x9C01040E, "r1 = (byte) dauc");            /* seen in the wild */

    /* ---- spc pseudo-instructions (hardware-verified words) ---- */
    ...
    expect(da(1, 5, 0, 1, 1, 0x3F, 0x2E, 0x07),
           "a1 = *r5-- - *r7++");                      /* FADD-STORE */
    expect(da(1, 6, 0, 1, 1, 0x0E, 0x03, 0x68),
           "a1 = (*r13 = a3) - *r1--");                /* FADD-TAP */
    expect(da(1, 4, 0, 0, 2, 0x00, 0x16, 0x08),
           "*r1 = a2 = *r2--");                        /* FLOAD-STORE */
    expect(da(3, 1, 0, 1, 0, 0x1B, 0x16, 0x0F),
           "*r1++ = a0 = a1 - *r2-- * *r3++r17");      /* FMULT-ACC-STORE */
    expect(da(2, 1, 1, 0, 0, 0x0F, 0x1B, 0x16),
           "a0 = -a1 + (*r2-- = *r3++r17) * *r1++");   /* FMULT-ACC-TAP */
    expect(da(1, 0, 0, 1, 2, 0x56, 0x77, 0x12),
           "*r2++r16 = a2 = *r14++ - a0 * *r10--");    /* FMULT-ADD-STORE */
    expect(da(3, 4, 0, 0, 3, 0x5E, 0x67, 0x13),
           "*r2++r17 = a3 = *r12++ * *r11--");         /* FMULT-STORE */
    expect(da(2, 4, 0, 1, 0, 0x46, 0x08, 0x12),
